...
class TimeKeeper:
                        ...
                        if framerate is not None and framerate > 0 and self.recording:
                            ...
                            with self.lock:
                                ...
                                if len(self.frame_durations) == self.window_size:
                                    ...
                                    sys.stdout.write(f"\rAverage frame rate (last {self.window_size} frames): {avg_framerate:.10f} fps")
                                    sys.stdout.flush()
    ...
    def adjust_frame_duration(self):
        ...
        learning_count = 0
        offset_adjustment_rate = 0.00001  # Rate at which to adjust the offset
        ...
        while self.is_running:
            try:
                ...
                with self.lock:
                    ...
                    # Apply offset to the calculated average framerate
                    adjusted_avg_framerate = avg_framerate + self.offset

                    # Calculate the error
                    error = self.target_framerate - adjusted_avg_framerate
                    ...
                    # Log the change
                    logger.info(f"Switched frame duration to {self.frame_duration} based on avg_framerate {adjusted_avg_framerate:.10f}, error {error:.10f}")

                    # Fine-tune the offset to home in on the target framerate
                    if abs(error) > 0.00000001:
                        self.offset += offset_adjustment_rate if error > 0 else -offset_adjustment_rate
                        logger.info(f"Adjusted offset to {self.offset} based on error {error:.10f}")
